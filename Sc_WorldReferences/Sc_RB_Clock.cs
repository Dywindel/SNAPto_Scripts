...
public class Sc_RB_Clock: MonoBehaviour {
		...
		else
		{
			...
			if (gM != null)
			{
				if ((cT_Hr_F_Norm >= dayLength_Day) && (cT_Hr_F_Norm < (1 - dayLength_Transition)))
				{
					...
					gM.DayNight_Transition_Trigger();
				}
				else
				{
	...
	// This is a generic timer function.
	// It will return true after a set amount of time has passed
}
