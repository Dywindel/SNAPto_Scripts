Shader "FX/Clouds" {
    ...
    SubShader
    {
        ...
        void disp(inout appdata_full v, out Input o)
        {
            ...
            float3 localPos = worldPos - mul(unity_ObjectToWorld, float4(0, 0, 0, 1)).xyz;// TEST local position for projecting
            ...
            // Swapped world position for local position
            float4 xy = float4((localPos.x * _Scale) - movementSpeedX, (localPos.y * _Scale)- movementSpeedY, 0, 0); // xy texture projection over worldpos * scale and movement
            float4 xz = float4((localPos.x * _Scale) - movementSpeedX, (localPos.z * _Scale) - movementSpeedZ, 0, 0); // same with xz
            float4 yz = float4((localPos.y * _Scale) - movementSpeedY, (localPos.z * _Scale) - movementSpeedZ, 0, 0); // same with yz
