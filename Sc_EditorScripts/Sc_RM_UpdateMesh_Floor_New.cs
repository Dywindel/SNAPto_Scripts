...
[ExecuteInEditMode]
public class Sc_RM_UpdateMesh_Floor_New : MonoBehaviour
{
    ...
    // This material list refers to the different normal maps that add detail to the meshes
    public Material[] ref_Material_NormalMaps;
    ...
    private List<Sc_RM_UpdateMesh_Floor_New> listOf_Ref_NeighbourMeshes = new List<Sc_RM_UpdateMesh_Floor_New>();
    ...
    public void RecordMeshNeighbour_AsBinary(int chainIndex)
    {
        ...
        if (chainIndex > 0)
        {
            ...
            foreach (Sc_RM_UpdateMesh_Floor_New temp_Script in listOf_Ref_NeighbourMeshes)
            {
                // If they still exist
                if (temp_Script != null)
        ...
        listOf_Ref_NeighbourMeshes = new List<Sc_RM_UpdateMesh_Floor_New>();
        ...
        for (int n = 0; n < rbV.cD; n++)
        {
            for (int m = 0; m < rbV.mD/rbV.cD; m++)
            {
                ...
                foreach (RaycastHit item in hitInfo)
                {
                    ...
                    {
                        // For reference
                        Sc_RM_UpdateMesh_Floor_New item_Script = item.collider.gameObject.GetComponent<Sc_RM_UpdateMesh_Floor_New>();
    ...
    void MeshDictionary(int n, int m = 0)
    {
        ...
        ref_MR.material = ref_Material_NormalMaps[n];
        this.transform.rotation = Quaternion.Euler(-90f, 45f*m, 0f);

        /*
        switch (n)
        {
            // For a solitary island
            case 0:
                ref_MeshFilter.mesh = ref_MeshFilter_Options[0];
                ref_MR.material = ref_Material_NormalMaps[0];
                break;
            // For a dead end
            case 1:
                // Update the mesh to our new shape
                ref_MeshFilter.mesh = ref_MeshFilter_Options[1];
                ref_MR.material = ref_Material_NormalMaps[1];
                // And rotate it by half of 90 (Because we're dividing the bin numbers by 2)
                this.transform.rotation = Quaternion.Euler(-90f, 45f*m, 0f);
                break;
            // For a bridge
            case 2:
                ref_MeshFilter.mesh = ref_MeshFilter_Options[2];
                ref_MR.material = ref_Material_NormalMaps[2];
                this.transform.rotation = Quaternion.Euler(-90f, 45f*m, 0f);
                break;
            // For a closed corner
            case 3:
                ref_MeshFilter.mesh = ref_MeshFilter_Options[3];
                ref_MR.material = ref_Material_NormalMaps[3];
                this.transform.rotation = Quaternion.Euler(-90f, 45f*m, 0f);
                break;
            // For a Tee junction
            case 4:
                ref_MeshFilter.mesh = ref_MeshFilter_Options[4];
                ref_MR.material = ref_Material_NormalMaps[4];
                this.transform.rotation = Quaternion.Euler(-90f, 45f*m, 0f);
                break;
            // For an open corner
            case 5:
                ref_MeshFilter.mesh = ref_MeshFilter_Options[5];
                this.transform.rotation = Quaternion.Euler(-90f, 45f*m, 0f);
                break;
            // For a four-way cross, which is symmetrical
            case 6:
                ref_MeshFilter.mesh = ref_MeshFilter_Options[6];
                break;
            // Weird T-ish pieces
            case 7:
                ref_MeshFilter.mesh = ref_MeshFilter_Options[7];
                this.transform.rotation = Quaternion.Euler(-90f, 45f*m, 0f);
                break;
            case 8:
                ref_MeshFilter.mesh = ref_MeshFilter_Options[8];
                this.transform.rotation = Quaternion.Euler(-90f, 45f*m, 0f);
                break;
            // An open edge
            case 9:
                ref_MeshFilter.mesh = ref_MeshFilter_Options[9];
                this.transform.rotation = Quaternion.Euler(-90f, 45f*m, 0f);
                break;
            // Other
            case 10:
                ref_MeshFilter.mesh = ref_MeshFilter_Options[10];
                this.transform.rotation = Quaternion.Euler(-90f, 45f*m, 0f);
                break;
            // Close inner corners
            case 11:
                ref_MeshFilter.mesh = ref_MeshFilter_Options[11];
                this.transform.rotation = Quaternion.Euler(-90f, 45f*m, 0f);
                break;
            // Far inner corners
            case 12:
                ref_MeshFilter.mesh = ref_MeshFilter_Options[12];
                this.transform.rotation = Quaternion.Euler(-90f, 45f*m, 0f);
                break;
            // One corner
            case 13:
                ref_MeshFilter.mesh = ref_MeshFilter_Options[13];
                this.transform.rotation = Quaternion.Euler(-90f, 45f*m, 0f);
                break;
            // Surrounded
            case 14:
                ref_MeshFilter.mesh = ref_MeshFilter_Options[14];
                break;
        }
        */
    }
